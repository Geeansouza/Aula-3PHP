	...
	*/
	...
	if(isset($_POST['btncalc'])){
		...
		if($_POST['txtn1'] == ""|| $_POST['txtn2'] == ""){
			...
			//validação de tratamento de erro rdo sem escolhar
			if(!isset($_POST['rdocalc'])){
				...
				if(!is_numeric($valor1) || !is_numeric($valor2)){
					echo('<script> alert ("Não é pssovel realizar calculos de daodos não numericos!);</script>');
					...
					switch ($opcao) {
						...
						case 'DIVIDIR':
							if($valor2 == 0)
								echo('<script> alert ("Não é possivel realizar divisão onde o valor 2 é igaul a 0!! ");</script>');
							else
								$resultado = ($valor1 / $valor2);
							break;
					default:
					// caso não entre em nenhuma das opçôes.
						}
					...
					// }	
					// //round() - permite limutar a quantidade de casas decimais de um valor
					// // além de arredondar o valor quando necessário 
					// // 
